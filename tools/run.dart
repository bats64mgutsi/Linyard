...
void main(List<String> args) async {
  ...
  if( args.length < 2 )
    error("At least two argumenta are required");
  ...
  Directory bin = Directory(mainScript.parent.parent.path+"/bin/$projectName");
  if(await bin.exists()){
    clearFolder(bin);
  } else {
...
void runWeb(Directory source, File mainScript, Directory bin) async {
  ...
  var build = () async {
    ...
    try{
      //TODO: The cmd /c command might not work on linux
      ...
      if(req.method == "GET"){
        ...
        if(file.existsSync()){
          ...
          if(filePath.endsWith(".html"))
            req.response.headers.add(HttpHeaders.contentTypeHeader, "text/html");
          else if(filePath.endsWith(".js"))
            req.response.headers.add(HttpHeaders.contentTypeHeader, "text/javascript");
